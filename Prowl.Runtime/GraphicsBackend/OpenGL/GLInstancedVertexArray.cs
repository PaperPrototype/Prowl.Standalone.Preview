...
/// <summary>
/// A vertex array that supports GPU instancing with both vertex and instance buffers.
/// </summary>
public sealed unsafe class GLInstancedVertexArray : GraphicsVertexArray
{
    public uint Handle { get; private set; }

    public GLInstancedVertexArray(
        VertexFormat vertexFormat, GraphicsBuffer vertices, GraphicsBuffer? indices,
        VertexFormat instanceFormat, GraphicsBuffer instanceBuffer)
    {
        Handle = GLDevice.GL.GenVertexArray();

        if (Handle == 0)
        {
            throw new System.Exception("Failed to create instanced VAO - glGenVertexArray returned 0");
        }
        ...
        BindFormat(vertexFormat);

        // Bind instance buffer and set up per-instance attributes
